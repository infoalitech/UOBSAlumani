...
class User {
    ...
    public function create($data) {
        // try {
            $stmt = $this->db->prepare("
                INSERT INTO users (name, email, password, role, active, is_alumni)
                VALUES (:name, :email, :password, :role, :active, :is_alumni)
            ");
            $stmt->execute([
                'name' => trim($data['name']),
                ...
                'is_alumni' => isset($data['is_alumni']) ? (int)$data['is_alumni'] : 1
            ]);
    ...
    public function read($id) {
        try {
            ...
            error_log("User::read - " . $e->getMessage());
            return null;
        }
    }
    ...
    public function update($id, $data) {
        try {
            $stmt = $this->db->prepare("
                UPDATE users 
                SET name = :name, email = :email, active = :active, is_alumni = :is_alumni 
                WHERE id = :id
            ");
            $stmt->execute([
                'id' => (int)$id,
    ...
    public function getUserCount($search = '') {
        try {
            ...
            error_log("User::getUserCount - " . $e->getMessage());
            return 0;
        }
    }
    ...
    public function getPaginatedUsers($limit, $offset, $search = '') {
        try {
            ...
            error_log("User::getPaginatedUsers - " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getUserPermissions($userId) {
        try {
            ...
            $totalPermissions = (int) $countStmt->fetch(PDO::FETCH_ASSOC)['total'];
            ...
            error_log("User::getUserPermissions - " . $e->getMessage());
            return [];
        }
    }
    ...
    public function assignPermissions($userId, $permissions) {
        try {
            ...
            error_log("User::assignPermissions - " . $e->getMessage());
            return false;
        }
    }
    ...
    public function clearUserPermissions($userId) {
        try {
            ...
            error_log("User::clearUserPermissions - " . $e->getMessage());
            return false;
        }
    }
    ...
    private function insertDefaultPermissions() {
        try {
            ...
            $this->db->rollBack();
            error_log("User::insertDefaultPermissions - " . $e->getMessage());
        }
    }
